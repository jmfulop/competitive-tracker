...
import {
  RefreshCw, Zap, Eye, MessageSquare, Activity,
  AlertTriangle, Clock, ChevronDown, ChevronRight, Radio, TrendingUp,
} from 'lucide-react';
  ...
  const handleScan = async () => {
    ...
    try {
      const res = await fetch('/api/signals/detect', {
        ...
        body: JSON.stringify({ vendor: scanVendor === 'all' ? vendors[0] : scanVendor }),
      });
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-6xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(s => {
            ...
            const isPat   = s.detected_by === 'ai_pattern';

            return (
              <div key={s.id} className={`bg-gray-900 rounded-xl border overflow-hidden transition-colors
                ${isPat ? 'border-purple-800/60' : 'border-gray-800 hover:border-gray-700'}`}>
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-1.5 mb-2">
                        ...
                        <span className={`flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${act.cls}`}>
                          {act.icon}{act.label}
                        </span>
                        <span className="text-xs text-gray-500">{s.vendor_tag}</span>
                        {s.source_type && (
                          <span className="text-xs text-gray-600">{SOURCE_LABEL[s.source_type] ?? s.source_type}</span>
                        )}
                      ...
                      <h3 className="text-sm font-semibold text-white mb-1">
                        {s.title || s.observation?.slice(0, 80)}
                      </h3>
                      ...
                      {s.roadmap_implication && (
                        <div className="mt-2 flex items-start gap-2">
                          ...
                          {s.roadmap_detail && (
                            <span className="text-xs text-gray-400">{s.roadmap_detail}</span>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="border-t border-gray-800 px-4 pb-4 pt-3 grid sm:grid-cols-2 gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                          <div className="h-full bg-indigo-500 rounded-full"
                            style={{ width: `${(s.confidence / 10) * 100}%` }} />
                        </div>
                    ...
                    <div>
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded border font-medium
                        ${s.detected_by === 'ai_pattern'
                          ? 'bg-purple-950 border-purple-700 text-purple-300'
                          : s.detected_by === 'ai_auto'
                            ? 'bg-indigo-950 border-indigo-800 text-indigo-300'
                            : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
                        {s.detected_by === 'ai_auto' ? 'AI auto-scan'
                          : s.detected_by === 'ai_pattern' ? 'AI pattern'
                          : 'Manual'}
                      </span>
                    </div>
                    {s.action_detail && (
      ...
      {digest && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-gray-900 border border-indigo-800 rounded-xl w-full max-w-xl p-6 max-h-[80vh] overflow-y-auto"
            ...
            {(digest.opportunities as string[])?.length > 0 && (
              <div className="bg-gray-800/50 rounded-lg p-4 mb-3">
                ...
                <ul className="space-y-1">
                  {(digest.opportunities as string[]).map((o, i) => (
                    <li key={i} className="text-sm text-gray-300 flex gap-2"><span>✓</span>{o}</li>
                  ))}
                </ul>
              </div>
            )}
            {(digest.watch_list as string[])?.length > 0 && (
              <div className="bg-gray-800/50 rounded-lg p-4">
                ...
                <ul className="space-y-1">
                  {(digest.watch_list as string[]).map((w, i) => (
                    <li key={i} className="text-sm text-gray-300 flex gap-2"><span>👁</span>{w}</li>
                  ))}
                </ul>
              </div>
            )}
