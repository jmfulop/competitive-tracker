...
function DeltaBadge({ previous, current }: {
  previous: Record<string, unknown> | null;
  current: Record<string, unknown> | null;
}) {
  if (!previous?.overall_score || !current?.overall_score) return null;
  const delta = +((current.overall_score as number) - (previous.overall_score as number)).toFixed(1);
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Every AI operation across the tracker</p>
        </div>
        <button onClick={() => load(page, true)}
                ...
                <div className="flex items-start gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      {Boolean(entry.meta?.model) && (
                        <span className="text-xs text-gray-700">{String(entry.meta?.model)}</span>
                      )}
                    </div>
                  </div>
                  {hasDiff && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      {entry.current && (
                        <div>
                          ...
                          <pre className="text-xs text-gray-300 bg-gray-950 rounded p-2 overflow-auto max-h-32">
