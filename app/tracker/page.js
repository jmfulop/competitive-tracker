...
export default function CompetitiveTracker() {
  ...
  const addSignal = async () => {
    ...
    setNewSignal(EMPTY_SIGNAL); await fetchSignals(); showToast('checkmark Signal logged successfully');
  };
  ...
  const saveEditedSignal = async () => {
    ...
    setEditingSignal(null); await fetchSignals(); showToast('checkmark Signal updated');
  };
                ...
                <ul className="space-y-2">
                  {validatedSignals.map(s => (
                    <li key={s.id} className={`${t.textSub} text-sm flex items-start gap-2`}>
                      <span className="text-green-500 mt-0.5">checkmark</span>
                    ...
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <div className="flex gap-2 shrink-0">
                        {signal.status === 'Monitoring' && <>
                          <button onClick={() => updateSignalStatus(signal.id, 'Validated')} className="bg-green-600/20 hover:bg-green-600/40 text-green-600 border border-green-600/30 text-xs px-3 py-1.5 rounded-lg transition-all">checkmark Validate</button>
                          <button onClick={() => updateSignalStatus(signal.id, 'Invalidated')} className={`${t.row} ${t.textSub} text-xs px-3 py-1.5 rounded-lg transition-all hover:opacity-80`}>x Dismiss</button>
                        </>}
                        {signal.status !== 'Monitoring' && (
